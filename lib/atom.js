...
function ATOM (options, items) {
    ...
    this.item = function (options) {
        ...
        if (this.limit !== undefined && this.items.length > this.limit)
            this.items.splice(0, this.items.length - this.limit);

        return item;
    };

    this.xml = function(indent, item) {
        return '<?xml version="1.0" encoding="UTF-8"?>\n'
                + XML(generateXML(this, item), indent);
    }
...
function generateXML (data, item){
    ...
    var pushItemFunction = function(item) {
        var entry = [
    ...
    if (item !== undefined) {
        // it works if item is a single item or an array of items
        [].concat(item).forEach(pushItemFunction);
    } else {
        // push all items
        data.items.forEach(pushItemFunction);
    }
